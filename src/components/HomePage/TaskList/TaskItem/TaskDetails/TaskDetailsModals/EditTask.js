...
const EditTask = ({open, setOpen, id, task}) => {
    ...
    return (
        ...
        >
            ...
            <Modal.Content>
                <div>
                    <label className={classes.label} htmlFor='category'>{t("category")}</label>
                ...
                <Form onSubmit={submitHandler}>
                    <Form.Field>
                        <label className={classes.label}>{t("newTitle")}</label>
                        <input type='text' id='title' minLength="10" maxLength="100" defaultValue={task.title}
                               ref={titleInput}/>
                    </Form.Field>
                    <Form.Field>
                        <label className={classes.label}>{t("newContent")}</label>
                        <input type='text' id='content' minLength="20" maxLength="800" defaultValue={task.content}
                               ref={contentInput}/>
                    </Form.Field>
                    <Form.Field>
                        <div>
                            <label className={classes.label} htmlFor='expirationDate'>{t("expDate")}</label>
                    ...
                    <Form.Field>
                        <label className={classes.label}>{t("newPay")}</label>
                        <input type='number' id='pay' maxLength="10" ref={payInput} defaultValue={task.pay}/>
                    </Form.Field>
                    <Form.Field>
                        <label className={classes.label}>{t("newEstTime")}</label>
                        <QuantityPicker onChange={getPickerValue} e min={1} max={24}
                                        value={task.estimatedTime}
                                        smooth/>
                    </Form.Field>
                    <Form.Field>
                        <label className={classes.label} htmlFor='address'>{t("findAddress")}</label>
                    ...
                    <div>
                        ...
                        {generatedAddress && <Message className={classes.addressMessage}>{t("selectedAddress")}{generatedAddress}</Message>}
                    </div>
